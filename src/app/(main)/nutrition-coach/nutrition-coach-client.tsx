...
import { generateCoachedDietPlan, testNutritionCoach, type NutritionCoachOutput } from "@/ai/flows/nutrition-coach";
...
export function NutritionCoachClient() {
  ...
  const formAction = async (formData: FullFormValues) => {
    startTransition(async () => {
      try {
        console.log('Generating nutrition plan for:', formData.profile);
        const result = await generateCoachedDietPlan(formData);
        console.log('Nutrition plan result:', result);
        
        if (result && result.diet_plan && result.detailed_notes) {
          setState(result);
          setCurrentStep(steps.length); // Move to result view
        } else {
          console.warn('Invalid nutrition plan result received');
          setState({ error: "Failed to generate complete diet plan. Please try again." });
        }
      } catch (e) {
  ...
  const testAI = async () => {
    try {
      const isWorking = await testNutritionCoach();
      if (isWorking) {
        toast({ title: "AI Test Passed", description: "Nutrition coach is working correctly" });
      } else {
        toast({ variant: "destructive", title: "AI Test Failed", description: "Nutrition coach is not responding properly" });
      }
    } catch (error) {
            ...
            <CardContent className="space-y-6">
                ...
                <Card className="bg-muted/50">
                    ...
                    <CardContent className="space-y-2 text-muted-foreground">
                        <p><strong>Breakfast:</strong> {state.diet_plan?.breakfast || 'Not specified'}</p>
                        <p><strong>Lunch:</strong> {state.diet_plan?.lunch || 'Not specified'}</p>
                        <p><strong>Snack:</strong> {state.diet_plan?.snack || 'Not specified'}</p>
                        <p><strong>Dinner:</strong> {state.diet_plan?.dinner || 'Not specified'}</p>
                        <p><strong>Hydration:</strong> {state.diet_plan?.hydration || 'Not specified'}</p>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-muted/50">
                     ...
                     <CardContent className="space-y-2 text-muted-foreground">
                        <p><strong>Target Calories:</strong> {state.detailed_notes?.calories || 'Not calculated'}</p>
                        <p><strong>Macronutrient Split:</strong> {state.detailed_notes?.macros || 'Not calculated'}</p>
                        <p><strong>Notes:</strong> {state.detailed_notes?.special_notes || 'No special notes'}</p>
                     </CardContent>
                </Card>

                <div className="flex gap-2">
                    <Button onClick={() => setCurrentStep(0)}>Start Over</Button>
                    <Button variant="outline" onClick={testAI}>Test AI</Button>
                </div>
            </CardContent>
        </Card>
     )
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{steps[currentStep]?.title || 'Nutrition Coach'}</CardTitle>
