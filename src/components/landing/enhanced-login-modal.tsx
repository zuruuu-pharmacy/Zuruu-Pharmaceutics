    ...
    options: [
      {
        id: "login",
        title: "Patient Login",
        description: "Access your existing patient profile",
        icon: LogIn,
      ...
      },
      {
        ...
        description: "Create a new patient history form",
        icon: UserPlus,
      ...
      },
      {
        ...
        description: "Immediately get assistance",
        icon: Siren,
    ...
    options: [
      {
        ...
        description: "Enter access code to continue",
        icon: Shield,
        color: "from-emerald-500 to-teal-500"
      }
    ]
...
interface EnhancedLoginModalProps {
  isOpen: boolean;
  onClose: () => void;
  role: 'patient' | 'pharmacist' | 'student';
  onSuccess: (role: string, action?: string) => void;
}

export function EnhancedLoginModal({ isOpen, onClose, role, onSuccess }: EnhancedLoginModalProps) {
  ...
  const handleAccessCodeVerification = async () => {
    ...
    if (accessCode === ACCESS_CODE) {
      ...
      toast({
        ...
        description: `Welcome to ${roleData.title}. Redirecting...`,
      });
  ...
  const handleOptionSelect = (optionId: string) => {
    if (optionId === 'access') {
      ...
      // For patient options, handle directly
      onSuccess(role, optionId);
      onClose();
    }
  };
        ...
        >
          ...
          <div className="relative p-8">
            ...
            >
              {/* Role Icon */}
              <motion.div
                className={`w-20 h-20 ${roleData.bgColor} ${roleData.borderColor} border-2 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-xl`}
              ...
              >
                <roleData.icon className={`w-10 h-10 bg-gradient-to-r ${roleData.color} bg-clip-text text-transparent`} />
              </motion.div>
              ...
              >
                <span className={`bg-gradient-to-r ${roleData.color} bg-clip-text text-transparent`}>
                  {roleData.title}
                </span>
              </motion.h2>
            ...
            >
              {!selectedOption ? (
                ...
                <div className="space-y-4">
                  {roleData.options.map((option, index) => (
                    <motion.button
                      key={option.id}
                      onClick={() => handleOptionSelect(option.id)}
                      className="w-full p-4 bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 hover:bg-white/20 transition-all duration-300 text-left"
